...
inline JSClassID JS_GetClassID2(JSValueConst val) {
    return JS_GetClassID(val, nullptr);
}
    ...
    struct js_traits<std::variant<Ts...>>
    {
        ...
        static bool isCompatible(JSContext * ctx, JSValueConst v) noexcept
        {
            ...
            switch(JS_VALUE_GET_TAG(v))
            {
                ...
                case JS_TAG_OBJECT:
                    ...
                    {
                        if(JS_GetClassID2(v) == js_traits<T>::QJSClassId)
                            return true;
                    }
                    return false;
        ...
        static std::variant<Ts...> unwrap(JSContext * ctx, JSValueConst v)
        {
            ...
            switch(tag)
            {
                ...
                case JS_TAG_OBJECT:
                    if(auto result = unwrapObj<Ts...>(ctx, v, JS_GetClassID2(v)))
                    {
                        return *result;
                    }
                    JS_ThrowTypeError(ctx, "Expected type %s, got object with classid %d",
                                      QJSPP_TYPENAME(std::variant<Ts...>), JS_GetClassID2(v));
                    break;
 ...
 */
    ...
    struct js_traits<std::shared_ptr<T>>
    {
        ...
        static std::shared_ptr<T> unwrap(JSContext * ctx, JSValueConst v)
        {
            ...
            auto obj_class_id = JS_GetClassID2(v);
 ...
 */
    ...
    public:
        ...
        template<typename T>
        std::function<T> GetFunction(std::string_view name) {
            std::function<T> f = eval(name).as<std::function<T>>();
            return f;
        }
